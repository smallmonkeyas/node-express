/*
 * @Author: your name
 * @Date: 2021-04-17 21:59:19
 * @LastEditTime: 2021-07-17 15:45:41
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: \basicService\省厅专网\历史数据导出\内网转换excel数据\getHIsData.js
 */
...
const basicSourceFolder =  __dirname+ "\\内网导出json数据";

const basicDirectFolder = __dirname + "\\内网转换excel数据";
 ...
 */
const dataJsonToMultiExcel = function (dataJsonArr, presentFactory) {
  //   let dataTrueJsonArr = []
  var excelJson;
  ...
  dataJsonArr.forEach((item) => {
    ...
    let param = item;
    excelJson = param[Object.keys(param)].reverse();
 ...
 */
const dataJsonToOneExcel = function (dataJsonArr, presentFactory) {
  //   let dataTrueJsonArr = []
  var excelJson, ws;
  ...
  //   XLSX.utils.book_append_sheet(wb, ws, paramArr[ii]);
  // }
  XLSX.writeFile(wb, presentFactoryInfoFolder + ".xlsx"); //直接定义死文件名
};

/**
 * @数据中对象键重复情况处理
 */
function dataKeysResolve(dataArr) {
  const dataKeys = [];
  const dataResolve = [];
  dataArr.forEach((item) => {
    const preKey = Object.keys(item)[0];
    if (dataKeys.includes(preKey)) {
      dataKeys.push(preKey + "_");
      // dataKeys.push.apply(dataKeys,[preKey+'_']);
    } else {
      dataKeys.push(preKey);
      // dataKeys.push.apply(dataKeys,[preKey]);
    }
    dataResolve.push({ [dataKeys[dataKeys.length - 1]]: item[preKey] });
  });
  return dataResolve;
}
/**
...
if (fs.existsSync(basicSourceFolder)) {
  ...
  for (var i = 0; i < files.length; i++) {
    ...
    var presentFactory = path.basename(
      presentSourceFilePath,
      path.extname(presentFile)
    );
    if (presentFactory !== "金坛二污0508-0610") continue;
    ...
    if (fileFactoryResult.length > 0) {
      ...
      var dataJsonResolveArr = dataKeysResolve(dataJsonArr);
      try {
        dataJsonToMultiExcel(dataJsonResolveArr, presentFactory);
        }catch (e) {
          sendMessage(`发送多个excel失败:${e}`)
        }
      try {
        dataJsonToOneExcel(dataJsonResolveArr, presentFactory);
        }catch (e) {
          sendMessage(`发送单个excel失败:${e}`)
